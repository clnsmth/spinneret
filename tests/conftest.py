...
@pytest.fixture
def test_geometry() -> list:
    """
    :return: A list of test geometries, as etree._ElementTree objects, from
        the `test_geometries.xml` file.
    """
    f = files("tests.data").joinpath("test_geometries.xml")
    xml = etree.parse(f)
    res = []
    for item in xml.xpath(".//geographicCoverage"):
